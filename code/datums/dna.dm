...
/datum/dna/proc/transfer_identity(mob/living/carbon/human/destination, transfer_SE = FALSE)
	...
	destination.flavour_text = destination.client.prefs.flavor_text //Update the flavor_text to use new dna text //NSV13
	if(transfer_SE)
	...
	if(prob(max(70-instability,0)))
		...
		switch(rand(0,5))
			...
			if(2)
				death()
				petrify(INFINITY)
				ghostize(FALSE) //NSV13 - Ethereal Crystalization
			if(3)
				if(prob(95))
