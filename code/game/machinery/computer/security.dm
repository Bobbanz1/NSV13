			...
			if("Print Record")
				if(!( printing ))
					...
					if((istype(active2, /datum/data/record) && GLOB.data_core.security.Find(active2)))
						...
						final_paper_text += "Criminal Status: [active2.fields["criminal"]]"
						//NSV13 - Roleplaying Records - End
						final_paper_text += "<BR>\n<BR>\nCrimes:<BR>\n"
...
//RECORD DELETE
			...
			if("Add Entry")
				...
				while(active2.fields["com_[counter]"])
					counter++
				active2.fields["com_[counter]"] = "Made by [src.authenticated] ([src.rank]) on [station_time_timestamp()] [time2text(world.realtime, "MMM DD")], [GLOB.year_integer+YEAR_OFFSET]<BR>[t1]" //NSV13 edit: year offset change
