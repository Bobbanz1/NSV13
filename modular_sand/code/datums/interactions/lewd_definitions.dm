...
/mob/living/proc/has_genital(slot, visibility = REQUIRES_ANY)
...
/mob/living/proc/has_hand(visibility = REQUIRES_ANY)
	if(iscarbon(src))
		var/mob/living/carbon/C = src
		var/handcount = 0
		var/covered = 0
...
/mob/living/proc/has_feet(visibility = REQUIRES_ANY)
	if(iscarbon(src))
		var/mob/living/carbon/C = src
		var/feetcount = 0
		var/covered = 0
...
/mob/living/proc/get_num_feet()
	return has_feet(REQUIRES_ANY)

/mob/living/proc/has_butt(visibility = REQUIRES_ANY)
	var/mob/living/carbon/C = src
		...
		if(!last_genital)
			if(has_penis())
				...
				switch(target_orifice)
					...
					if(CUM_TARGET_HAND)
						if(partner.has_hand(REQUIRES_ANY))
