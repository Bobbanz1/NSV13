...
/datum/interaction/lewd
	...
	//REQUIRES_ANY for both exposed and unexposed.
		...
		switch(require_user_penis)
			if(REQUIRE_EXPOSED)
				if(!user.has_penis(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!user.has_penis(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_user_balls)
			if(REQUIRE_EXPOSED)
				if(!user.has_balls(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!user.has_balls(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_user_vagina)
			if(REQUIRE_EXPOSED)
				if(!user.has_vagina(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!user.has_vagina(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_user_breasts)
			if(REQUIRE_EXPOSED)
				if(!user.has_breasts(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!user.has_breasts(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_target_penis)
			if(REQUIRE_EXPOSED)
				if(!target.has_penis(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!target.has_penis(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_target_balls)
			if(REQUIRE_EXPOSED)
				if(!target.has_balls(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!target.has_balls(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_target_anus)
			if(REQUIRE_EXPOSED)
				if(!target.has_anus(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!target.has_anus(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_target_vagina)
			if(REQUIRE_EXPOSED)
				if(!target.has_vagina(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!target.has_vagina(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_target_breasts)
			if(REQUIRE_EXPOSED)
				if(!target.has_breasts(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!target.has_breasts(REQUIRES_ANY))
					if(!silent)
		...
		switch(require_target_feet)
			if(REQUIRE_EXPOSED)
				if(!target.has_feet(REQUIRE_EXPOSED))
					...
					return FALSE
			if(REQUIRES_ANY)
				if(!target.has_feet(REQUIRES_ANY))
					if(!silent)
